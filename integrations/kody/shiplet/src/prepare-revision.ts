...
/**
 * Fork, edit and validate a candidate while keeping the active review unchanged.
 ...
 * @example
 * import run from 'kody:@your-account/shiplet/prepare-revision'
 * const result = await run(input)
 */
export default async function run(
  input: Parameters<ReturnType<typeof client>["prepareRevision"]>[0],
  connection?: Connection,
) {
  return client(connection).prepareRevision(input);
}
