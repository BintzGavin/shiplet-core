...
/**
 * Recover an existing project’s current immutable package and revision.
 ...
 * @example
 * import run from 'kody:@your-account/shiplet/inspect'
 * const result = await run(input)
 */
export default async function run(
  input: Parameters<ReturnType<typeof client>["inspect"]>[0],
  connection?: Connection,
) {
  return client(connection).inspect(input);
}
