...
/**
 * Inspect an acknowledged draft checkpoint before recovering an interrupted preparation.
 ...
 * @example
 * import run from 'kody:@your-account/shiplet/inspect-draft'
 * const result = await run(input)
 */
export default async function run(
  input: Parameters<ReturnType<typeof client>["inspectDraft"]>[0],
  connection?: Connection,
) {
  return client(connection).inspectDraft(input);
}
