...
| DOC-043 | in_scope | Organization and team invite links are taught with their limits, deliberate member-only joining, plain refusals, and audit records.                    | Both access-guide renderers teach the Workspace Invite links section, organization or team targets, use, expiry, and reserved-email settings, copy and revoke, deliberate member-only joining, and repeat opens that take no extra use. The vitest suite `test/organization-invite-links.spec.ts` covers administrator-only management, limits, case-insensitive email reservation, repeat opens, member-only joins, and audited create, redeem, and revoke; the `/join/:token` page states cover sign in, accept, joined, wrong account, and revoked, expired, or used-up refusals. Both route families are classified in `api-route-ledger.json`.        | pass   | Re-run the production workspace and join-link smoke after an authorized deployment.                                        |
