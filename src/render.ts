...
.shiplet-brand-header[data-header-variant="authenticated"] {
  background: color-mix(in oklch, var(--surface), transparent 7%);
}

.shiplet-brand-header[data-header-variant="public"] {
...
.shiplet-brand-mark .shiplet-mark-vessel,
.shiplet-brand-mark .shiplet-mark-water-motion {
...
.shiplet-brand-mark .shiplet-mark-rigging { color: var(--mark-ink); opacity: 0.58; }
.shiplet-brand-mark .shiplet-mark-deck-line,
.shiplet-brand-mark .shiplet-mark-hull-chine { color: var(--surface); }
...
/* Ambient motion has no playback UI. Visibility and the system motion
   preference gate both the bounded surface sampler and native detail motion. */
...
.shiplet-waterline-drawn { stroke-dashoffset: 0; }
.shiplet-waterline-far { color: var(--mark-harbor); opacity: 0.46; }
.shiplet-waterline-mid { color: var(--mark-harbor); opacity: 0.65; }
.shiplet-waterline-near { color: var(--mark-harbor); opacity: 0.9; }
.shiplet-waterline-body { fill: currentColor; fill-opacity: 0.2; stroke: none; }
.shiplet-waterline-foam { color: color-mix(in oklch, var(--mark-harbor), white 62%); opacity: 0.86; stroke: currentColor; stroke-width: 1.1; }
.shiplet-waterline-tertiary { opacity: 0.46; }
.shiplet-waterline-marker-buoy { opacity: 0.78; }
.shiplet-waterline-marker-buoy .shiplet-waterline-buoy-body { fill: var(--action); stroke: var(--action); }
.shiplet-waterline-marker-buoy .shiplet-waterline-buoy-stem,
.shiplet-waterline-marker-buoy .shiplet-waterline-buoy-topmark { fill: none; stroke: var(--mark-ink); stroke-linecap: round; stroke-linejoin: round; }
.shiplet-brand-header[data-header-variant="public"] .shiplet-waterline-svg { opacity: 0.88; }
...
@media (max-width: 900px) {
  ...
  .url-tag { display: none; }
    ...
    .shiplet-brand-inner { padding: 6px 16px; }
...
const BRAND_MARK_SVG = `<svg viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" shape-rendering="geometricPrecision">
  <g class="shiplet-mark-position" transform="translate(0 -8)">
    ...
    <g class="shiplet-mark-vessel" data-header-vessel="primary" data-harbor-motion>
      ...
      <path d="M28 86h72l-13 24H41z" fill="var(--mark-ink, #20293a)"/>
      <path class="shiplet-mark-deck-line" d="M35 87h58" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" opacity=".88"/>
      <path class="shiplet-mark-hull-chine" d="M43 101h42" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" opacity=".42"/>
...
function HeaderWaterlineSvg(variant: HeaderVariant) {
  ...
  const variantDetail =
    variant === "authenticated"
      ...
      <g class="shiplet-waterline-avatar-position"><g class="shiplet-waterline-wave shiplet-waterline-avatar-ripple shiplet-waterline-tertiary" data-harbor-motion fill="none" stroke="currentColor" stroke-width="1.25" stroke-linecap="round">
...
export const HARBOR_SCENE_SVG = `<svg class="harbor-scene-svg" viewBox="0 0 640 190" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" shape-rendering="geometricPrecision" fill="none" stroke="currentColor" stroke-width="2.35" stroke-linecap="round" stroke-linejoin="round">
  ...
  <g class="scene-midground scene-supporting-details">
    <g class="scene-dock scene-detail">
      ...
      <g class="scene-dock-mooring scene-tertiary-detail">
        <path class="draw-path scene-mooring-coil" style="--di:4" d="M205 139c9-5 17 1 12 7-4 5-15 3-12-3 3-5 15-4 18 5" pathLength="1"/>
        <path class="draw-path scene-mooring-rope" style="--di:4" d="M217 148c7 1 11 5 15 10" pathLength="1"/>
      </g>
    </g>
    ...
    <g class="scene-beacon scene-detail">
      ...
      <g class="scene-beacon-masonry scene-tertiary-detail">
        <path class="draw-path" style="--di:3" d="M520 115h14m8 0h17m8 0h4" pathLength="1"/>
        <path class="draw-path" style="--di:3" d="M518 135h11m9 0h16m10 0h8" pathLength="1"/>
      </g>
  ...
  <g class="scene-foreground scene-water">
    <g class="scene-foreground-tide">
      ...
      <g class="scene-water-mid-layer">
        ...
        <g class="scene-beacon-water-glints scene-beacon-reflection">
          <path class="draw-path scene-fine-detail scene-intentional-contour" style="--di:4" d="M492 169c9-3 18-3 27 0s18 3 27 0" pathLength="1"/>
          <path class="draw-path scene-fine-detail scene-intentional-contour" style="--di:4" d="M536 183c8-2 16-2 24 0s16 2 24 0" pathLength="1"/>
        </g>
      </g>
    ...
    <g class="scene-boat-arrival scene-boat scene-working-vessel">
      ...
      <g class="scene-boat-float">
        ...
        <g class="scene-vessel-working-details scene-fine-detail">
          <path class="draw-path scene-antenna scene-structural-line" style="--di:3" d="M384 84V72m-5 5h10" pathLength="1"/>
          <path class="draw-path scene-deck-rail" style="--di:3" d="M401 122v-9h10v9" pathLength="1"/>
          <circle class="draw-path scene-life-ring" style="--di:3" cx="398" cy="114" r="4" pathLength="1"/>
        </g>
...
(function () {
	...
	d.addEventListener("DOMContentLoaded", function () {
		...
		if (header && water) {
			...
			var wake = header.querySelector(".shiplet-brand-wake-extension");
			var buoy = water.querySelector(".shiplet-waterline-marker-buoy-position");
			var avatarWater = water.querySelector(".shiplet-waterline-avatar-position");
			...
			function drawWater(still) {
				waterModels.forEach(function (model, index) {
					...
					if (index === 1 && vessel) {
						var heave = still ? 0 : frame.heave * waterScale;
						vessel.style.transform = still ? "" : "translateY(" + (heave * 3.2).toFixed(2) + "px) rotate(" + (frame.pitch * 0.5).toFixed(2) + "deg)";
						if (contact) contact.style.translate = still ? "" : "0 " + (heave * 3.2).toFixed(2) + "px";
			...
			function syncHeaderMotion() {
				...
				var paused = prefersStill || d.hidden || !headerInView || suspended;
				header.setAttribute("data-header-motion", paused ? "paused" : "running");
				if (paused) {
			...
			function measureWater() {
				var bounds = water.getBoundingClientRect();
				var mark = header.querySelector(".shiplet-brand-mark").getBoundingClientRect();
				waterWidth = Math.max(1, Math.round(bounds.width));
				waterScale = bounds.height / 40;
				shipX = mark.left + mark.width / 2 - bounds.left;
				buoyX = Math.max(shipX + 170, waterWidth * 0.25);
				water.setAttribute("viewBox", "0 0 " + waterWidth + " 40");
				if (avatarWater) avatarWater.setAttribute("transform", "translate(" + (waterWidth - 1200) + " 0)");
				drawWater(!motionPreference || motionPreference.matches);
			}
			measureWater();
			if (window.ResizeObserver) new ResizeObserver(measureWater).observe(header);
			else window.addEventListener("resize", measureWater);
			d.addEventListener("visibilitychange", syncHeaderMotion);
			window.addEventListener("pagehide", function () { suspended = true; syncHeaderMotion(); });
...
export function renderPage(
  ...
  ${HeaderWaterlineSvg(headerVariant)}
</header>`;
