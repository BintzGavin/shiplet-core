...
function getAuthHeaders(env: Env): Record<string, string> {
	// This legacy custom-hostname adapter is bounded to the configured zone. It
	// must never inherit the retired dispatch deployment credential.
...
function isApiConfigured(env: Env): boolean {
	const hasAuth = !!env.CLOUDFLARE_API_TOKEN;
